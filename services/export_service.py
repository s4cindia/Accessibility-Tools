...
def _outpath(folder: Path, stem: str, ext: str) -> Path:
    # Write into the current login session's private subfolder so each session
    # only ever sees (and can only download) the files it generated.
    from utils.file_utils import session_subdir
    folder = session_subdir(folder)
...
def export_issues_pdf(flat_issues: list[dict[str, Any]],
                      numbering: dict[str, Any] | None = None,
                      file_name: str = "", stem: str = "issues_report") -> Path:
    """Render the Issues panel (integrity + numbering problems) as a PDF.

    *flat_issues* is a list of ``{"row", "col", "msg"}`` rows (one per problem,
    exactly as shown in the Issues panel). *numbering* is the optional
    check-id-sequence payload (missing / duplicated / mismatched numbers).
    """
    total = len(flat_issues)
    seq = numbering or {}
    missing = seq.get("missing") or []
    dups = seq.get("duplicates") or []
    mism = seq.get("mismatches") or []
    seq_total = len(missing) + len(dups) + len(mism) if seq.get("active") else 0

    parts = ["<h1>Issues Report</h1>",
             f"<p class='meta'>Generated {_dt.datetime.now():%Y-%m-%d %H:%M}"
             + (f" &middot; {html.escape(file_name)}" if file_name else "")
             + f" &middot; {total + seq_total} issue(s)</p>"]

    if not flat_issues and not seq_total:
        parts.append("<div class='score'>No issues found — the sheet is "
                     "consistent. 🎉</div>")
    else:
        if flat_issues:
            rows = "".join(
                f"<tr><td>{html.escape(str(i.get('row') or ''))}</td>"
                f"<td>{html.escape(str(i.get('col') or '—'))}</td>"
                f"<td>{html.escape(str(i.get('msg') or ''))}</td></tr>"
                for i in flat_issues)
            parts.append(f"<h2>Data integrity — {total} problem(s)</h2>")
            parts.append("<table class='data'><thead><tr><th>Row</th>"
                         "<th>Column</th><th>Problem</th></tr></thead>"
                         f"<tbody>{rows}</tbody></table>")
        if seq_total:
            first_col = html.escape(str(seq.get("first_col") or ""))
            parts.append(f"<h2>Numbering — column “{first_col}” "
                         f"(should run 1–{html.escape(str(seq.get('max')))})</h2>")
            if missing:
                parts.append(f"<p class='note'><b>Missing ({len(missing)}):</b> "
                             f"{html.escape(', '.join(str(n) for n in missing))}</p>")
            if dups:
                parts.append(f"<p class='note'><b>Duplicated ({len(dups)}):</b> "
                             f"{html.escape(', '.join(str(n) for n in dups))}</p>")
            if mism:
                mrows = "".join(
                    f"<tr><td>{html.escape(str(m.get('row') or ('#' + str(m.get('id')))))}</td>"
                    f"<td>{html.escape(str(m.get('id_num')))}</td>"
                    f"<td>{html.escape(str(m.get('summary_num')))}</td></tr>"
                    for m in mism)
                parts.append(f"<p class='note'><b>{first_col} number ≠ Summary "
                             f"number ({len(mism)} row(s)):</b></p>")
                parts.append("<table><thead><tr><th>Row</th>"
                             f"<th>{first_col}</th><th>Summary #</th></tr></thead>"
                             f"<tbody>{mrows}</tbody></table>")

    out = _outpath(config.report_dir, stem, ".pdf")
    _html_to_pdf("".join(parts), out, landscape=True, n_cols=3)
    return out

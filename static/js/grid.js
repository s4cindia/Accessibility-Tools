...
(function () {
    ...
    const cols = (SDE.columnDefs || []).map((c) => {
      ...
      const def = {
        ...
        cellClassRules: {
          ...
          // Summary cells longer than 250 chars or containing a line break
          "summary-overflow-cell": (p) => isSummary && p.value != null &&
            (String(p.value).length > 250 || String(p.value).indexOf("\n") >= 0),
          "missing-cell": (p) => {
            const v = p.value;
            return v === null || v === undefined ||
              (typeof v === "string" && v.trim() === "");
          },
          // Placeholder 1 (merge): any cell mentioning "Best Practice" /
          // "Best Practise" / "best-practice" (hyphen, e.g. an axe Tags cell) is
          // highlighted so the user can update it.
          "best-practice-cell": (p) => window.SDE_PAGE_MODE === "merge" &&
            p.value != null && /best[\s\-]*practi[sc]e/i.test(String(p.value)),
        },
      };
      if (isNum) {
  ...
  /* Placeholder 1 (merge): clicking a highlighted "Best Practice" cell opens a
     popup asking the user to update it (with a shortcut to the row editor). */
  const _BEST_PRACTICE_RE = /best[\s\-]*practi[sc]e/i;
  function onBestPracticeCellClicked(ev) {
    if (window.SDE_PAGE_MODE !== "merge") return;
    if (!ev || !ev.colDef || !ev.data) return;
    const v = ev.value;
    if (v == null || !_BEST_PRACTICE_RE.test(String(v))) return;
    const rowNo = (ev.rowIndex != null) ? ev.rowIndex + 1 : null;
    if (typeof Swal === "undefined") {
      SDE.toast("This cell contains a “Best Practice” item — please update it.", "warn");
      return;
    }
    Swal.fire({
      icon: "warning", title: "Best Practice — please update",
      html: `<div style="text-align:left;color:#475569">This cell (column `
        + `<b>${SDE.esc(ev.colDef.field)}</b>) contains a <b>Best Practice</b> entry. `
        + `Please review and update it before delivery.</div>`,
      showCancelButton: true, confirmButtonText: "Edit row", cancelButtonText: "Close",
      confirmButtonColor: "#4f46e5",
    }).then((res) => {
      if (res && res.isConfirmed) openRowEditor(ev.data, ev.colDef.field, rowNo);
    });
  }
  ...
  function gridOptions() {
    return {
      ...
      rowClassRules: {
        ...
        "parent-flag-row": (p) => p.data && G.parentIds.has(p.data.__id),
      ...
      onCellClicked: onBestPracticeCellClicked,
      ...
      buttons: [
        ...
        { label: "Save row", variant: "primary", onClick: async () => {
            ...
            try {
              ...
              // Live integrity re-check (delivery: WCAG/blanks/numbering;
              // merge: numbering) after saving the row.
              _recheckWcag();
            } catch (e) {
  ...
  async function onCellValueChanged(ev) {
    ...
    try {
      ...
      SDE.refreshStatus();
      // Live integrity re-check after ANY cell edit (delivery: WCAG/blanks/
      // numbering; merge: numbering). The backend re-reads the data and the
      // Issues/Blanks badges update.
      _recheckWcag();
    } catch (e) {
  ...
  // Re-run the live integrity checks after ANY data change (no popups; results
  // land on the Issues/Blanks badges + panels). Delivery: WCAG + blanks +
  // numbering. Merge: WCAG + numbering.
  function _recheckWcag() {
    // Re-validate live on every edit so changes (incl. Description) update the
    // Issues panel/badge and row highlights immediately. The page still loads
    // clean because afterOpen does not auto-validate on load.
    if ((window.SDE_PAGE_MODE === "delivery" || window.SDE_PAGE_MODE === "merge")
        && SDE.checkWcag) SDE.checkWcag();
    if ((window.SDE_PAGE_MODE === "delivery" || window.SDE_PAGE_MODE === "merge")
        && SDE.checkIdSequence) SDE.checkIdSequence();
    if (window.SDE_PAGE_MODE === "delivery" && SDE.checkParentFormat)
      SDE.checkParentFormat();
  }
  ...
  G.reload = async function () {
    ...
    if (!G.built) { G.init(); _recheckWcag(); return; }
  ...
  G.setParentFlagRows = function (ids) {
    G.parentIds = new Set(ids || []);
    if (G.api) G.api.redrawRows();
  };
  ...
  // Toggle the server-side "only rows with blank cells" filter (delivery editor).
  G.setBlanksOnly = function (on) {
    G.blanksOnly = !!on;
    G.refresh();
  };
  ...
  SDE.actions["add-row"] = async function () {
    ...
    try {
      ...
      G.refresh();
      _recheckWcag();   // keep the Issues badge count in sync with row count

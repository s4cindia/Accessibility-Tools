...
(function () {
  ...
  SDE.actions["validate"] = async function () {
    ...
    try {
      ...
      loadErrorHighlights();
      // Placeholder 3: this click is what turns on issue/blank detection.
      // Run the WCAG/spec checks + blanks now, and enable live re-checking so
      // flags clear as the user fixes them.
      SDE.validateRan = true;
      if (window.SDE_PAGE_MODE === "delivery") {
        if (SDE.checkWcag) await SDE.checkWcag();           // also runs checkBlanks
        if (SDE.checkIdSequence) await SDE.checkIdSequence();
      }

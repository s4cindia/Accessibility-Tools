...
def download(area: str, filename: str):
    ...
    # Generated files (exports/reports) live in the current session's private
    # subfolder, so a session can only download its OWN outputs. Uploads are
    # shared source files and stay as-is.
    if area in ("exports", "reports"):
        folder = file_utils.session_subdir(folder)

...
def load_tabular(path: Path, sheet: str | None = None, max_cols: int = 18,
    ...
    if validate_audit:
        from services.feature_service import (validate_audit_columns,
                                              normalize_issue_category_header)
        # Column C is sometimes labelled 'Issue Type' — normalise it to
        # 'Issue Category' (by position) before validating.
        df = normalize_issue_category_header(df)
    ...
    # NOTE: the placeholder-3 (delivery) auto-fill of a blank 'Issue Category'
    # from the row's 'Issue' value was removed — that conversion condition now
    # lives in placeholder 2 (axe -> audit), which fills Issue Category upstream
    # so files arriving here already have it populated.
    # Strip a leading hyphen from 'Description' on EVERY load (any placeholder),
    # so a file that already contains one shows a clean, editable Description in
    # the grid immediately. A leading '-' makes Excel treat the text as a
    # formula; the conversion no longer creates one (placeholder 2), and this is
    # the load-time safety net for older files.
    df = feature_service.strip_description_hyphen(df)
    if validate_audit:
        # Placeholder 3 (delivery): add readable blank lines between the
        # Description's sections so the loaded sheet is easy to read.
        df = feature_service.space_description_sections(df)
    if sort_id:
    ...
    store.add_recent(path, sheet)  # per-session recent list (in memory)
...
def load_pdf(path: Path) -> None:
    ...
    store.add_recent(path)  # per-session recent list (in memory)

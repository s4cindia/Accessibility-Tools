...
def session_subdir(base: Path) -> Path:
    """Return a per-login-session subfolder of *base* (created on demand), so a
    session's generated outputs are isolated from every other session and can be
    deleted wholesale when the session ends. Falls back to *base* itself outside
    a request/login context (e.g. CLI)."""
    base = Path(base)
    try:
